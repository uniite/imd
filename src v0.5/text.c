...
int8 drawFont(int8 fontIndex, int8 scale, int8 startX, int8 startY, int8 r, int8 g, int8 b, int1 inverse) 
{ 
    int8 bit, col; 
    int1 singleBit; 
    int8 X1, Y1, X2, Y2;
    int8 i, i2;
	// Width of char and running count of empty pixels to  find out width
	int8 width, empty_count;
	width = 0;
	empty_count = 0;
    ...
    for(bit=0; bit<13; bit++) 
    {
        for (i = 0; i < scale; i ++) {
            ...
            { 
                // lets get the bit 
                singleBit=(font_arial[fontIndex][bit] & bitMask8[7-col])>>col; 
			...
			// Check if this row's width is larger than the previous
			if (8 - empty_count > width) {
				// If so record it as the largest row so far (representing the width of the whole character)
				width = 8 - empty_count;
			}
			empty_count = 0;
        }
    }
	//printf("Charwidth (%u): %u\n", fontIndex, width);
	return width;
}

//kidan useless as is..ill fixit later
...
int8 oled_textbox(int8 x, int8 y, char text[1000], int8 fontsize, int8 maxwidth, int8 r, int8 g, int8 b, int8 mode, int8 align, int1 inverse) {
    ...
    if (mode != 3) {
        if (inverse) {
            //oled_fill(x, y, screenwidth, y + charheight, r, g, b);
			for (i = 0; i < maxwidth; i ++) {
	                    curx += drawFont(getIndex(text[i]), fontsize, x + curx, y, oled_bg, OFF);
						// For spcaing
						curx ++;
						if (text[i] == 95) {
							// For space (chr 32d)
            ...
            //oled_fill(x, y, screenwidth, y + charheight, oled_bg);
			for (i = 0; i < maxwidth; i ++) {
	                    curx += drawFont(getIndex(text[i]), fontsize, x + curx, y, r, g, b, OFF);
						// For spcaing
						curx ++;
						if (text[i] == 95) {
							// For space (chr 32d)
		...
		// Multiline text..one of the many things I have to convert to use the new arial  font before it's usable again
        ...
        /*for (i = 0; i < lines; i ++) {
            ...
            if (inverse) {
                for (i2 = 0; i2 < maxwidth; i2 ++) {
                    curx += drawFont(getIndex(text[(i * maxwidth) + i2]), fontsize, x + curx, y + (i * charheight), oled_bg, OFF);
					// For spcaing
					curx ++;
					if (text[(i * maxwidth) + i2] == 32) {
						// For space (chr 32d)
						curx += 2;
					}
                }
                ...
                for (i2 = 0; i2 < maxwidth; i2 ++) {
                    curx += drawFont(getIndex(text[(i * maxwidth) + i2]), fontsize, x + curx, y + (i * charheight), r, g, b, OFF);
					// For spcaing
					curx ++;
					if (text[(i * maxwidth) + i2] == 32) {
						// For space (chr 32d)
